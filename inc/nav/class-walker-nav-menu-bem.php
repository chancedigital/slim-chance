...
class Walker_Nav_Menu_Bem extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = [], $id = 0 ) {
		 ...
		 */
		$args = apply_filters( 'slim_chance_nav_menu_bem_item_args', $args, $item, $depth );
		 ...
		 * @param int      $depth   Depth of menu item. Used for padding.
		 */
		$class_names = join( ' ', apply_filters( 'slim_chance_nav_menu_bem_css_class', array_filter( $classes ), $item, $args, $depth ) );
		 ...
		 * @param int      $depth   Depth of menu item. Used for padding.
		 */
		$id = apply_filters( 'slim_chance_nav_menu_bem_item_id', 'menu-item-' . $item->ID, $item, $args, $depth );
		 ...
		 * @param int      $depth Depth of menu item. Used for padding.
		 */
		$atts = apply_filters( 'slim_chance_nav_menu_bem_link_attributes', $atts, $item, $args, $depth );
		...
		$title = apply_filters( 'slim_chance_bem_the_title', $item->title, $item->ID );
		 ...
		 * @param int      $depth Depth of menu item. Used for padding.
		 */
		$title = apply_filters( 'slim_chance_nav_menu_bem_item_title', $title, $item, $args, $depth );
		 ...
		 * @param stdClass $args        An object of wp_nav_menu() arguments.
		 */
		$output .= apply_filters( 'slim_chance_walker_nav_menu_bem_start_el', $item_output, $item, $depth, $args );
	}
	...
	public function start_lvl( &$output, $depth = 0, $args = [] ) {
		...
		$class_names = join( ' ', apply_filters( 'slim_chance_nav_menu_bem_submenu_css_class', $classes, $args, $depth ) );
		$class_names = $class_names ? ' class="' . esc_attr( $class_names ) . '"' : '';
